...
shinyUI(navbarPage("WHO Data Set Visualization",
	tabPanel("Home",
		sidebarPanel(
			...
			p("A video tutorial is available at ", 
...
tabPanel("Analysis",
	fluidRow(
		...
		column(4,
		selectInput("spVar1", label = "Variable 1", choices = colnames(WHO.subset)), #selectInput 
		selectInput("spVar2", label = "Variable 2", choices = colnames(WHO.subset)) #selectInput 
		),
		column(5,
